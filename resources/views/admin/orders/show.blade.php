...
@section('content')
    ...
    <div class="ksm-grid ksm-grid--2">
        ...
        <section class="ksm-card" style="padding: 20px;">
            <h2 style="font-size: 1.05rem;">Dati</h2>
            <ul class="ksm-meta">
                <li>Cliente: {{ $order->billing_name }}</li>
                <li>Email: {{ $order->billing_email }}</li>
                <li>Indirizzo: {{ $order->billing_address }}, {{ $order->billing_city }} {{ $order->billing_zip }}</li>
            ...
            <form method="POST" action="{{ route('admin.orders.status', $order) }}" style="margin-top: 16px; display: flex; gap: 8px;">
                @csrf @method('PATCH')
